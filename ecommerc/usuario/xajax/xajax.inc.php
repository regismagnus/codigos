...
class xajax
{
	...
	var $sEncoding;
	/**
	 * @var boolean Decode input request args from ISO-8859-1 (default false)
	 */
	 ...
	 * <i>Usage:</i> <kbd>$xajax->setCharEncoding("ISO-8859-1");</kbd>
	 *
	 * @param string the encoding type to use (ISO-8859-1, iso-8859-1, etc.)
	 */
	...
	/**
	 * Causes xajax to decode the input request args from ISO-8859-1 to the current
	...
	/**
	 * Turns off decoding the input request args from ISO-8859-1 (default behavior).
	 */
	...
	/**
	 * Decodes string data from ISO-8859-1 to the current xajax encoding.
	 * 
	...
	function _decodeUTF8Data($sData)
	{
		...
		if ($this->bDecodeUTF8Input)
		{
			...
			{
				trigger_error("The incoming xajax data could not be converted from ISO-8859-1", E_USER_NOTICE);
			}
			...
			if ($sFuncToUse)
			{
				if (is_string($sValue))
				{
					if ($sFuncToUse == "iconv")
					{
						$sValue = iconv("ISO-8859-1", $this->sEncoding.'//TRANSLIT', $sValue);
					}
					else if ($sFuncToUse == "mb_convert_encoding")
					{
						$sValue = mb_convert_encoding($sValue, $this->sEncoding, "ISO-8859-1");
					}
					else
					{
